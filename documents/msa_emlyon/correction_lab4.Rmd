...
= & \; \mathbb{E}\left[(a + \sum_{i=1}^n \omega_ix_i - a)^2\right], \\
= & \; \mathbb{E}\left[ \left(\sum_{i=1}^n \omega_ix_i\right)^2\right], \\
= & \; \mathbb{E}\left[ \sum_{i=1}^n \left(\omega_ix_i\right)^2 +2  \sum_{i<i'}^n \omega_i\omega_{i'}x_ix_{i'}\right], \\
= & \;  \sum_{i=1}^n \underbrace{\mathbb{E}\left[\omega_i^2\right]}_{= Var[\varepsilon_i] = \sigma^2}x_i^2 +2  \sum_{i<i'}^n \underbrace{\mathbb{E}\left[\omega_i\omega_{i'}\right]}_{=0}x_ix_{i'},\\
...
\[\hat{\sigma}^2 =\dfrac{1}{n-2} \sum_{i=1}^n \varepsilon_i ^2 =\dfrac{1}{n-2}\sum_{i=1}^n \left(y_i - \bar{y}\right)^2. \]

Au final notre variance de l'estimateur $\hat{a}$ est alors donnée par : 

\[\dfrac{\dfrac{1}{n-2}\sum_{i=1}^n \left(y_i - \bar{y}\right)^2}{\sum_{i=1}^n(x_i-\bar{x})^2}\]
...
\[t = \dfrac{\hat{a} - a}{\sqrt{\dfrac{\dfrac{1}{n-2}\sum_{i=1}^n \left(y_i - \bar{y}\right)^2}{\sum_{i=1}^n(x_i-\bar{x})^2}}} \underset{\text{sous}\; H_0}{=} \dfrac{\hat{a}}{\sqrt{\dfrac{\dfrac{1}{n-2}\sum_{i=1}^n \left(y_i - \bar{y}\right)^2}{\sum_{i=1}^n(x_i-\bar{x})^2}}}. \]
...
\[ \left[ \hat{a}-t_{1-\alpha/2, n-2}\sqrt{\dfrac{\dfrac{1}{n-2}\sum_{i=1}^n \left(y_i - \bar{y}\right)^2}{\sum_{i=1}^n(x_i-\bar{x})^2}}, \hat{a} + t_{1-\alpha/2, n-2}\sqrt{\dfrac{\dfrac{1}{n-2}\sum_{i=1}^n \left(y_i - \bar{y}\right)^2}{\sum_{i=1}^n(x_i-\bar{x})^2}}\right].\]
