...
Inutile de faire un test de Shapiro pour voir que les réisdus ne sont pas gaussiens. 
Regardons à nouveau avec le log des données 
...
La variable qualitative prenant plus que deux modalités, nous devrons faire une Analyse de Variance, on vérifiera les hypothèses de l'ANOVA a posteriori 

```{r}
modele = aov(X2~X17,data)
```

```{r}
qqnorm(modele$residuals)
qqline(modele$residuals)
shapiro.test(modele$residuals) # taille ech
```
